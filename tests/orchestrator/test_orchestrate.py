...
""")
    ...
    assert r.stdout.index("Vague parallèle : T1") < r.stdout.index("Vague parallèle : T2")
...
def test_happy_run_with_auto_checkpoint(sandbox: Path) -> None:
    (sandbox / ".shim" / "T1.sh").write_text('echo "données" > t1.txt\necho "STATUS: done"\n')
    write_tasks(sandbox, """
...
""")
    ...
    assert state(sandbox) == {"T1": "done", "CP-1": "done", "T2": "done", "CP-2": "done"}
    ...
    import subprocess
    log = subprocess.run(["git", "log", "--oneline"], cwd=sandbox, capture_output=True, text=True).stdout
...
""")
    ...
    assert state(sandbox) == {
        "T1": "done", "T2": "blocked", "T3": "skipped", "CP-1": "skipped"
    }
