...
Every environment variable read by the lab's `scripts/`. Variables are read at
...
| `LAB_ROOT` | repo root (`scripts/..`) | Data/config root: model registry, golden files, outputs. Override for tests. Framework code is resolved via `__file__`, not this. | `orchestrate.py`, `eval_models.py` |
...
(`scripts/sandbox_runner.py`) are forwarded into the container.
